...
public class TinyAppWidget extends AppWidgetProvider {
	...
	private static final String[] PROJECTION_APPWIDGETS = new String[] { AppWidgetsColumns.UNITS, };

	private static final int COL_UNITS = 0;

	private static final String[] PROJECTION_FORECASTS = new String[] {
			ForecastsColumns.CONDITIONS, ForecastsColumns.TEMP_HIGH,
	...
	public void onDeleted(Context context, int[] appWidgetIds) {
		...
		for (int appWidgetId : appWidgetIds) {
			...
			Uri appWidgetUri = ContentUris.withAppendedId(
					AppWidgets.CONTENT_URI, appWidgetId);
	...
	public static RemoteViews buildUpdate(Context context, Uri appWidgetUri) {
		...
		try {
			Uri forecastAtUri = Uri.withAppendedPath(appWidgetUri,
					AppWidgets.TWIG_FORECAST_AT);
			Uri forecastAtNowUri = Uri.withAppendedPath(forecastAtUri,
					Long.toString(System.currentTimeMillis()));
			cursor = resolver.query(forecastAtNowUri, PROJECTION_FORECASTS,
					null, null, null);
			if (cursor != null && cursor.moveToFirst()) {

				String conditions = cursor.getString(COL_CONDITIONS);
				int iconResource = ForecastUtils.getIconForForecast(conditions,
						daytime);
				...
				if (tempHigh == Integer.MIN_VALUE
						|| tempLow == Integer.MIN_VALUE) {
		...
		if (!forecastFilled) {
			...
			views.setTextViewText(R.id.loading,
					res.getString(R.string.widget_error));
		}

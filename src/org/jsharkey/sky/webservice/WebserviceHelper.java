...
public class WebserviceHelper {
	...
	private static final String[] PROJECTION_APPWIDGET = {
			AppWidgetsColumns.LAT, AppWidgetsColumns.LON,
	 ...
	 */
	public static void updateForecasts(Context context, Uri appWidgetUri,
		...
		Uri appWidgetForecasts = Uri.withAppendedPath(appWidgetUri,
				AppWidgets.TWIG_FORECASTS);
		...
		resolver.delete(appWidgetForecasts, ForecastsColumns.VALID_START
				+ " >= " + earliest + " OR " + ForecastsColumns.VALID_START
				+ " <= " + lastMidnight, null);

		// Insert any new forecasts found
		ContentValues values = new ContentValues();
		for (Forecast forecast : forecasts) {
			Log.d(TAG, "inserting forecast with validStart="
					+ forecast.validStart);
			values.clear();
